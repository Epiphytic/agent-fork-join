...
The plugin can be disabled temporarily or permanently:

**Environment Variable (temporary):**
```bash
export FORK_JOIN_DISABLED=1  # Disable entirely
export FORK_JOIN_NO_AI=1     # Disable AI, use heuristics only
```

**Settings File (persistent):**
```bash
mkdir -p ~/.config/agent-fork-join
echo '{ "enabled": false }' > ~/.config/agent-fork-join/settings.json
```

Priority (highest to lowest):
1. `FORK_JOIN_DISABLED=1` environment variable
2. `~/.config/agent-fork-join/settings.json` with `{ "enabled": false }`
3. Default: enabled
