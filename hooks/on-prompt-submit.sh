...
main() {
	...
	if [[ "$on_main" == "true" ]]; then
		...
		if [[ -n "$existing_pr" ]]; then
			...
			# Add beads comment about updated PR and sync to JIRA
			if [[ -n "$current_issue" ]]; then
				local pr_url
				pr_url=$(cache_get_pr_url 2>/dev/null || echo "")

				# Truncate prompt for comment (first 100 chars)
				local prompt_preview="${PROMPT:0:100}"
				if [[ ${#PROMPT} -gt 100 ]]; then
					prompt_preview="${prompt_preview}..."
				fi

				local beads_comment="New work started on PR #${existing_pr}:

**PR URL:** ${pr_url}
**Prompt:** ${prompt_preview}
**JIRA Ticket:** ${jira_key:-N/A}

---
_Automated comment from agent-fork-join_"

				if beads_add_comment "$current_issue" "$beads_comment" 2>/dev/null; then
					debug_log "Added beads comment for PR update"
				fi
